...
if __name__ == '__main__':
    ...
    experiments = [
        ...
        ('gemm_complex',testing1toNParam, [(5, 5, 8), [('m', lambda N: N), ('k', lambda N: N), ('n', lambda N: N)] ], 'gemm_complex-4-124-24') ,
...
#        ('symgen' , testingNParam , [ [('m' , 12 , 12, 4)] ] , 'dbg_symgen-5-5-0') ,
#         ('symgen_complex' , testingNParam , [ [('m' , 8 , 8, 4)] ] , 'comp_symgen-4-4-0') ,
#        ('ugem' , testingNParam , [ [('m' , 7, 7, 4)] ] , 'testing-12')
    ...
    common_config = parse_config_file('%s/temp_common.conf' % MAIN_CONFIG_FOLDER)
    ...
    for experiment in experiments:
        ...
        for dev in devices:
            dev_config_file = '%s/devices/temp_%s.conf' % (MAIN_CONFIG_FOLDER, dev)
            precomp_series = []
            precomp_markers = []
            precomp_linestyles = []
            precomp_colors = []
            ...
            precomp_order = []
            for lib in libs:
                lib_config_file = '%s/libs/temp_%s.conf' % (MAIN_CONFIG_FOLDER, lib)
